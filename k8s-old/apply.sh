...
GCP_SA="prometheus-stackdriver@anthos-quest.iam.gserviceaccount.com"
...
kubectl create ns store -o yaml --dry-run=client | kubectl apply -f -

kubectl annotate serviceaccount -n store default iam.gke.io/gcp-service-account=${GCP_SA} --overwrite

kubectl apply -f ${STORE_CONFIGMAP_FILE} -n store
kubectl apply -f ${STORE_DEPLOYMENT_FILE} -n store
